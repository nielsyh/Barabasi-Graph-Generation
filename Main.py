...
def generateModelA(N, m):
    ...
    for r in range(0,N-m):
        ...
        for i in range(0, m):
            ...
            if (not G.has_edge(new_node, random_node)):
...
def generateModelBA(N, m):
    ...
    # construct N-m new nodes
    for r in range(0, N - m):
        # print('amount g: '+ str(len(G.nodes)))
        current_size = len(G.nodes)
        total_edges = len(G.edges)*2
        arr = []
        for node in range(0, current_size):
            arr.append(node)
        for node in range(0, current_size):
            #print(str(node) + 'has: ' + str(G.edges(node)))
            arr[node] = len(G.edges(node)) / total_edges

        #print(str(arr))
        links = np.random.choice(current_size, m, p=arr)
        new_node = (m + r)
        G.add_node(new_node)
        for node in links:
            #if(node != new_node):
...
def frequencies(G):
    ...
    max_degree = max(deg)
    freq = list(map(lambda x: float(x) / N, cnt))
